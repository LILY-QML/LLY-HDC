...
\centering
\begin{quantikz}
\lstick{$q_0$} & \gate{R_y(\theta_F)} & \gate{R_z(\theta_F/2)} & \ctrl{3} & \qw & \qw & \ctrl{6} & \qw \\
\lstick{$q_1$} & \gate{R_y(\theta_F)} & \qw & \qw & \ctrl{3} & \qw & \qw & \ctrl{6} & \qw \\
\lstick{$q_2$} & \gate{R_y(\theta_F)} & \qw & \qw & \qw & \ctrl{3} & \qw & \qw & \ctrl{6} & \qw \\
\lstick{$q_3$} & \qw & \qw & \targ{} & \gate{R_y(\theta_{FH}^0)} & \qw & \qw & \ctrl{3} & \qw & \meter{} \\
\lstick{$q_4$} & \qw & \qw & \qw & \targ{} & \gate{R_y(\theta_{FH}^1)} & \qw & \qw & \ctrl{3} & \qw & \meter{} \\
\lstick{$q_5$} & \qw & \qw & \qw & \qw & \targ{} & \gate{R_y(\theta_{FH}^2)} & \qw & \qw & \ctrl{3} & \meter{} \\
\lstick{$q_6$} & \qw & \qw & \targ{} & \gate{R_z(\theta_{FG}^0)} & \qw & \qw & \targ{} & \gate{R_y(\theta_{HG}^0)} & \meter{} \\
\lstick{$q_7$} & \qw & \qw & \qw & \targ{} & \gate{R_z(\theta_{FG}^1)} & \qw & \qw & \targ{} & \gate{R_y(\theta_{HG}^1)} & \meter{} \\
\lstick{$q_8$} & \qw & \qw & \qw & \qw & \targ{} & \gate{R_z(\theta_{FG}^2)} & \qw & \qw & \targ{} & \gate{R_y(\theta_{HG}^2)} & \meter{}
\end{quantikz}
\caption{Parametrized quantum circuit architecture showing the three subsystems and their entangling operations.}
\label{fig:circuit}
\end{figure}
...
\addplot[blue, thick] coordinates {
    (0, 2.5) (5, 1.8) (10, 1.2) (15, 0.8) (20, 0.5) 
    (25, 0.35) (30, 0.25) (35, 0.18) (40, 0.15) (45, 0.12) (50, 0.1)
};
...
\begin{axis}[
    width=0.45\textwidth,
    height=0.35\textwidth,
    xlabel={Input (Freshness)},
    ylabel={Output Value},
    grid=major,
    legend pos=north west
]
\addplot[red, thick, smooth] coordinates {
    (1, 1.8) (2, 3.2) (3, 4.5) (4, 5.8) (5, 7.0) 
    (6, 7.8) (7, 8.5) (8, 8.8) (9, 9.2) (10, 9.5)
};
\addlegendentry{Durability}
\addplot[green, thick, smooth] coordinates {
    (1, 2.0) (2, 3.5) (3, 5.2) (4, 6.8) (5, 8.0) 
    (6, 8.8) (7, 9.2) (8, 8.9) (9, 8.2) (10, 7.5)
};
\addlegendentry{Taste}
\end{axis}
\end{tikzpicture}
...
\node[box] (input) at (0,0) {Primary\\Sequence};
\node[box] (encode) at (4,0) {Quantum\\Encoding};
\node[box] (vqc) at (8,0) {VQC\\Processing};
\node[box] (output) at (12,0) {Properties:\\Stability, Binding,\\Conformation};

\draw[arrow] (input) -- (encode);
\draw[arrow] (encode) -- (vqc);
\draw[arrow] (vqc) -- (output);

\node[below=0.5cm of encode] {$\ket{\psi_{\text{seq}}}$};
\node[below=0.5cm of vqc] {$U(\vec{\theta})\ket{\psi}$};
\end{tikzpicture}
\caption{Proposed pipeline for protein property prediction using quantum circuits.}
\label{fig:protein_pipeline}
\end{figure}
